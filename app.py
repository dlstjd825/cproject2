...
# is_user_mentor 함수 불러오기
def is_user_mentor(user_id):
    user_id_bytes = user_id.encode('utf-8')
    return c_function.is_user_mentor(user_id_bytes)  # C 함수 호출
...
# load_classes 함수 정의(이후에 C로 구현할 수도 있음)
    ...
    try:
        with open('static/classes.csv', 'r', encoding='utf-8') as csvfile:
            for line in csvfile:
                ...
                if len(fields) == 9:  # 필드 개수 확인
                    # 클래스 정보를 딕셔너리 형태로 추가
                    class_title, mentor_name, location, class_description, price, mode, capacity, current_applicants, image_path = fields # C 함수 호출
                    classes.append({
...
def create_class():
    ...
    image_path = os.path.join('images/profile', image_filename).replace("\\", "/")
    ...
    with open('static/classes.csv', 'a', encoding='utf-8') as csvfile:
        csvfile.write(f"{class_title},{mentor_name},{location},{class_description},"
                      f"{price},{mode},{capacity},{current_applicants},{image_path}\n")
    ...
    if success:
        ...
        if len(details) == 9:
            ...
            class_details = {
                ...
                'current_applicants': int(details[7]),
...
def apply_class(class_title):
    ...
    with open('static/applications.csv', 'a', newline='', encoding='utf-8') as csvfile:
        ...
        writer.writerow([class_title, applicant_name, email, datetime.now().strftime("%Y-%m-%d %H:%M:%S")])
